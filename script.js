...
if (resultForm) {
  resultForm.addEventListener('submit', function (e) {
    ...
    const courseClass = document.getElementById('resultClass').value;

    if (rollNo && courseClass) {
      // Simulate processing
      resultOutput.style.display = 'none';
      resultOutput.innerHTML = '<i class="fa fa-spinner fa-spin me-2"></i> Fetching result...';
      resultOutput.style.display = 'block';
      resultOutput.className = 'mt-3 alert alert-info'; // Bootstrap alert style

      setTimeout(() => {
        // Mock Logic: Even roll numbers pass, odd fail (just for demo)
        // In reality, this would be an API call.
        const isPass = rollNo % 2 === 0;
        const marks = isPass ? Math.floor(Math.random() * (400 - 300) + 300) : Math.floor(Math.random() * (250 - 150) + 150);
        const total = 500;
        const status = isPass ? "PASSED" : "SUPPLY";
        const colorClass = isPass ? "alert-success" : "alert-danger";
        const icon = isPass ? "fa-check-circle" : "fa-exclamation-circle";

        resultOutput.className = `mt-3 alert ${colorClass}`;
        resultOutput.innerHTML = `
                    <h5 class="alert-heading"><i class="fa ${icon}"></i> Result Status: ${status}</h5>
                    <p class="mb-0">
                        <strong>Roll No:</strong> ${rollNo} <br>
                        <strong>Class:</strong> ${courseClass} <br>
                        <strong>Marks Obtained:</strong> ${marks} / ${total}
                    </p>
                `;
      }, 1500);
    }
